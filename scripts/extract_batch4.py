"""Fetch Batch 4 Shopify product pages and dump title/tags/body_html/vendor
for each handle so we can reconcile botanical data across retailers.

Batch 4: Azaleas + Nandina + Fruit Trees (6 plants)
  - autumn-royalty-encore-azalea (FGT, NH, PT)
  - autumn-twist-encore-azalea   (FGT, NH)
  - autumn-angel-encore-azalea   (FGT, PT)
  - nandina-heavenly-bamboo      (FGT, NH, PT)  <- NH handle is 'Firepower' dwarf cultivar, cross-check species
  - santa-rosa-plum              (FGT, NH, PT)
  - dwarf-cavendish-banana       (FGT, PT)

FGT JSON endpoints 404 - skip them (same as Batch 3).
"""
import json
import sys
import os
...
OUT_FILE = PROJECT_ROOT / "data" / "batch4_extraction_results.json"
