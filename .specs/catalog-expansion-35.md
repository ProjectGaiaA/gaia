...
- [ ] `ADDING_PLANTS.md` exists and documents the full process
- [ ] `scrapers/extract_plant_data.py` exists with tests, produces valid plant entries
- [ ] All candidates that passed the 2-retailer cut are active in `plants.json`
- [ ] All candidates that failed the cut are documented and remain inactive (or removed)
- [ ] Reactivation candidates (Vinca Minor, Echinacea PowWow) activated if 2+ handles confirmed, otherwise remain inactive
- [ ] `handle_maps.json` has entries for every active new plant × every retailer that carries it
- [ ] `python -X utf8 build.py` succeeds with zero errors
- [ ] Every new active plant has a rendered product page in `site/plants/`
- [ ] Every affected category page shows updated plant counts
- [ ] `site/sitemap.xml` includes all new product page URLs
- [ ] No existing plant pages or functionality broken by the additions
